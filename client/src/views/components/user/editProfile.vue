        ...
        <v-card-text>
            ...
            <v-form class="px-3" ref="form">
                <v-layout wrap align-center justify-start >
                    ...
                    <v-flex xs12 sm6 >
                        <v-text-field label="用户名" v-model="username" prepend-icon="person"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="真实姓名" v-model="realname" prepend-icon="person"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-select v-model="genderselect" :items="genderitems" label="性别" prepend-icon="wc"></v-select>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :nudge-right="40" lazy transition="scale-transition" offset-y full-width min-width="290px">
                            <template v-slot:activator="{ on }">
                                <v-text-field v-model="date" label="出生日期" prepend-icon="event" readonly v-on="on"></v-text-field>
                            </template>
                            <v-date-picker ref="picker" v-model="date" :max="new Date().toISOString().substr(0, 10)" min="1950-01-01" @change="savedate"></v-date-picker>
                        </v-menu>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-select v-model="nationselect" :items="nationitems" label="民族" prepend-icon="home"></v-select>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="籍贯" v-model="nativeplace" prepend-icon="place"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-select v-model="politicalselect" :items="politicalitems" label="政治面貌" prepend-icon="group"></v-select>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="学院" v-model="college" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="年级" v-model="grade" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="学制" v-model="majorset" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="培养类别" v-model="eduset" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="电话" v-model="phone" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm6>
                        <v-text-field label="邮箱" v-model="email" prepend-icon="school"></v-text-field>
                    </v-flex>
                    <v-flex xs12 sm12>
                        <v-textarea label="教育背景（高中起）" v-model="eduback" prepend-icon="edit"></v-textarea>
                    </v-flex>
                    <v-flex xs12 sm12>
                        <v-textarea label="研究生学生工作经历" v-model="ug_exp" prepend-icon="edit"></v-textarea>
                    </v-flex>
                    <v-flex xs12 sm12>
                        <v-textarea label="其他阶段学生工作经历" v-model="other_exp" prepend-icon="edit"></v-textarea>
                    </v-flex>
                </v-layout>
                <v-spacer></v-spacer>
